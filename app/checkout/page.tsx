        ...
        {toasts.map((t) => (
          <motion.div
            ...
            className={`pointer-events-auto flex items-start gap-3 px-4 py-3
              rounded-xl shadow-xl border min-w-[260px] max-w-[340px]
              ${t.type === "error"   ? "bg-gray-900 border-red-500/40"   : ""}
              ${t.type === "success" ? "bg-gray-900 border-green-500/40" : ""}
              ${t.type === "info"    ? "bg-gray-900 border-yellow-500/40": ""}
            `}
          >
            <span className="mt-0.5 shrink-0">
              {t.type === "error"   && <FiAlertCircle  size={15} className="text-red-400"   />}
              {t.type === "success" && <FiCheckCircle  size={15} className="text-green-400" />}
              {t.type === "info"    && <FiAlertCircle  size={15} className="text-yellow-400"/>}
            </span>
            <p className="text-white text-xs leading-relaxed flex-1">{t.message}</p>
            <button
...
function useToast() {
  ...
  return { toasts, remove, toast: show };
}
...
interface CartItemType {
  ...
  quantity:      number;
  image:         string;
  color?:        string;   // ✅ needed for variant lookup
  variantIndex?: number;   // ✅ needed for variant lookup
}
...
const JazzCashLogo = () => (
  <svg viewBox="0 0 80 28" fill="none" className="h-7 w-auto">
    ...
    <text x="8" y="20" fontFamily="Arial Black, sans-serif" fontWeight="900" fontSize="14" fill="white">Jazz</text>
    <text x="40" y="20" fontFamily="Arial Black, sans-serif" fontWeight="900" fontSize="14" fill="#FFD700">Cash</text>
  </svg>
);
...
const EasypaisaLogo = () => (
  <svg viewBox="0 0 90 28" fill="none" className="h-7 w-auto">
    ...
    <text x="8" y="19" fontFamily="Arial Black, sans-serif" fontWeight="900" fontSize="11" fill="#4CAF50">e</text>
    <text x="26" y="20" fontFamily="Arial Black, sans-serif" fontWeight="900" fontSize="12" fill="white">easypaisa</text>
  </svg>
);
        ...
        <div className="relative rounded-2xl overflow-hidden bg-gray-900 aspect-square">
          {error ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 p-6 text-center">
              ...
              <button onClick={handleRetry} className="px-4 py-2 bg-yellow-500 text-gray-900 rounded-lg text-sm font-bold">
                Try Again
              </button>
            </div>
          ) : (
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="relative w-48 h-48">
                  {[
                    ...
                    <div key={`corner-${i}`} className={`absolute w-8 h-8 border-yellow-400 rounded-sm ${cls}`} />
                  ))}
                  <motion.div
        ...
        {scanning && !("BarcodeDetector" in window) && (
          <div className="mt-3 bg-amber-500/10 border border-amber-500/25 rounded-xl px-4 py-3">
            <p className="text-amber-400 text-xs text-center">
              Auto-detection not supported on this browser. Use your JazzCash / EasyPaisa app to scan instead.
            </p>
          </div>
        )}
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="bg-white rounded-2xl p-4 flex flex-col items-center gap-3">
          <div className="relative w-36 h-36">
            <Image
              src={qrSrc} alt={`${label} QR`} fill className="object-contain" sizes="144px"
              onError={(e) => {
          ...
          <p className="text-gray-500 text-[10px] text-center font-medium">Scan with {label} app</p>
        </div>

        <div className="flex flex-col gap-3">
          <div className="bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 flex-1">
            <p className="text-[10px] text-gray-500 uppercase tracking-widest mb-1">{label} Number</p>
          ...
          <div className={`${bgClass} border ${borderClass} rounded-xl px-4 py-3`}>
            <p className="text-[10px] text-gray-400 uppercase tracking-widest mb-0.5">Amount to Send</p>
            <p className={`${color} font-bold text-lg`}>Rs. {displayTotal}</p>
          </div>

          <button
            type="button" onClick={onOpenScanner}
      ...
      <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-4">
        <p className="text-[11px] font-semibold text-gray-500 uppercase tracking-widest mb-3">How to pay</p>
...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    const stored = JSON.parse(localStorage.getItem("cart") || "[]") as CartItemType[];
    setCart(stored);
  }, []);
  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    try {
      const payload: Record<string, unknown> = {
        ...form,
        paymentMethod,
        // ✅ FIX: color and variantIndex are now included so the API can
        //         find the correct variant and not return 400.
        products: cart.map((i) => ({
          product:      i._id,
          quantity:     i.quantity,
          color:        i.color,
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-5">
                ...
                <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-4">
                  ...
                  <select name="country" value={form.country} onChange={handleChange} className={inputClass}>
                    {["Pakistan", "United States", "United Kingdom", "Canada", "Australia", "Germany", "France"].map((c) => (
                  ...
                  <AnimatePresence mode="wait">
                    {paymentMethod === "cod" && (
                      <motion.div
                        ...
                        initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: 8 }} transition={{ duration: 0.2 }}
                        className="bg-yellow-500/10 border border-yellow-500/25 rounded-2xl px-5 py-4 flex items-start gap-3"
                      >
                    ...
                    {paymentMethod === "jazzcash" && (
                      <motion.div key="jazzcash" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} transition={{ duration: 0.2 }}>
                        <ScanPanel
                    ...
                    {paymentMethod === "easypaisa" && (
                      <motion.div key="easypaisa" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} transition={{ duration: 0.2 }}>
                        <ScanPanel
                    ...
                    {paymentMethod === "card" && (
                      ...
                      >
                        <div className="relative h-44 rounded-2xl overflow-hidden
                          ...
                          <div className="w-10 h-7 bg-yellow-400/80 rounded-md grid grid-cols-2 gap-0.5 p-1">
                          ...
                          <div>
                            ...
                            <div className="flex justify-between mt-1.5">
                              ...
                              <p className="text-gray-400 text-xs">{cardForm.expiry || "MM/YY"}</p>
                            </div>
                          </div>
                        </div>

                        <input
                          name="holder" value={cardForm.holder} onChange={handleCardChange}
                          placeholder="Card Holder Name" required={paymentMethod === "card"} className={inputClass}
                        />
                        <input
                          name="number" value={cardForm.number} onChange={handleCardChange}
                          placeholder="Card Number" maxLength={19} required={paymentMethod === "card"}
                          className={inputClass + " font-mono tracking-widest"}
                        />
                        <div className="grid grid-cols-2 gap-3">
                          <input
                            name="expiry" value={cardForm.expiry} onChange={handleCardChange}
                            placeholder="MM/YY" maxLength={5} required={paymentMethod === "card"} className={inputClass}
                          />
                          <input
                            name="cvc" value={cardForm.cvc} onChange={handleCardChange}
                            placeholder="CVC" maxLength={3} required={paymentMethod === "card"} className={inputClass}
                          />
                        </div>
                        <p className="text-[11px] text-gray-600">🔒 Your card details are encrypted and secure</p>
                      </motion.div>
                    )}
                  </AnimatePresence>

                  {scannedData && (paymentMethod === "jazzcash" || paymentMethod === "easypaisa") && (
                    <div className="bg-green-500/10 border border-green-500/25 rounded-xl px-4 py-3 flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-gray-500 text-[11px] font-mono mt-0.5 break-all">{scannedData}</p>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-4">
                <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-4">
                    ...
                    {cart.map((item) => (
                      <div key={item._id} className="flex items-center gap-3">
                        <div className="relative w-12 h-12 rounded-lg overflow-hidden bg-gray-800 flex-shrink-0">
                          <Image
                            src={item.image || "/placeholder.jpg"} alt={item.title}
                            fill className="object-cover" sizes="48px"
                          />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-white text-xs font-medium truncate">{item.title}</p>
                          {/* ✅ show color badge if present */}
                          <div className="flex items-center gap-1.5 mt-0.5">
                            <p className="text-gray-500 text-xs">x{item.quantity}</p>
                            {item.color && (
                              <span className="text-[10px] text-gray-400 bg-gray-700/60 px-1.5 py-0.5 rounded-md">
                                {item.color}
                              </span>
                            )}
                          </div>
                  ...
                  <div className={`${active.bg} border ${active.border} rounded-xl px-4 py-2.5 flex items-center justify-between`}>
